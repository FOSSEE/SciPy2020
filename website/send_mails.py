...
from Scipy2020.settings import LOG_FOLDER
...
def send_email(request, call_on,
    ...
    if call_on == "Registration":
        message = dedent("""\
                    Thank you for registering for SciPy India 2020.
        ...
        try:
            send_mail(
                "User Registration - SciPy India 2020, FOSSEE, IIT Bombay", message, SENDER_EMAIL,
                [request.user.email], fail_silently=True
            )

        except Exception:
            send_smtp_email(request=request,
                            subject="User Registration - SciPy India 2020, FOSSEE, IIT Bombay",
